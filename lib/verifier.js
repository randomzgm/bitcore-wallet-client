 ...
 */
Verifier.checkAddress = function(credentials, address, sha256) {
  $.checkState(credentials.isComplete());

  var local = Utils.deriveAddress(address.type || credentials.addressType, credentials.publicKeyRing, address.path, credentials.m, credentials.network, credentials.coin, sha256);
...
Verifier.checkTxProposalSignature = function(credentials, txp) {
  ...
  if (!Verifier.checkAddress(credentials, txp.changeAddress, (txp.coldStakingAddress ? true : false)))
    return false;
