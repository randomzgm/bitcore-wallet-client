...
API.prototype._getHeaders = function(method, url, args) {
  ...
  if (args['kongAuth']) {
    Object.assign(headers, self._kongAuth(args['kongAuth']))
  }
...
API.prototype._kongAuth = function(auth) {
  let headers = {}
  if (auth['method'] === 'hmac') {
    const gmTime = moment.utc().format('ddd, DD MMM YYYY HH:mm:ss') + ' GMT';
    const hmac = crypto.createHmac('sha256', auth['secret']);
    const signature = hmac.update(util.format('date: %s', gmTime), 'utf-8').digest('base64');
    headers = {
      'Authorization': util.format('hmac username=\"%s\", algorithm=\"hmac-sha256\", headers=\"date\", '
