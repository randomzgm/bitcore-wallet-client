...
API.prototype.recreateWallet = function(cb) {
  ...
  this.getStatus({
    ...
    var args = {
      ...
      coldStakingAddress: c.coldStakingAddress
    };
 ...
 * setupColdStaking
 *
 * @param opts.coldStakingAddress - Either the cold staking bech32 address or xpub key
 * @param cb - Returns error
 * @return {undefined}
 */
API.prototype.setupColdStaking = function(opts, cb) {
  var self = this;
  self._doPutRequest('/v1/coldstaking/', opts, function(err, res){
    if (err) return cb(err);
    
    self.credentials.setupColdStaking(opts.coldStakingAddress);
