...
API.prototype.openWallet = function(opts, cb) {
  ...
  self._doGetRequestWithAuth('/v2/wallets/?includeExtendedInfo=1', opts, function(err, ret) {
    if (err) return cb(err);
...
API.prototype._getHeaders = function(method, url, args) {
  ...
  if (args['kongAuth']) {
    let auth = args['kongAuth'];
    // 认证后立即删除，以免影响后续逻辑
    delete args['kongAuth'];
    
    Object.assign(headers, self._kongAuth(auth, args, method));
  }
...
API.prototype._kongAuth = function(auth, args, method) {
  let headers = {};
  if (auth['method'] === 'hmac') {
    const gmTime = moment.utc().format('ddd, DD MMM YYYY HH:mm:ss') + ' GMT';
    const hash = crypto.createHash('sha256');
    let body = method === 'get' ? '' : '{}'; // 不同的提交方法有不同的初始值
    // TODO 需要有大量的测试用例验证body的生成方法是否与实际一致
    if (_.isString(args)) {
      body = qs.stringify(args);
    } else if (Object.keys(args).length) {
      body = JSON.stringify(args);
    }
    const bodyDigest = util.format('SHA-256=%s', hash.update(body, 'utf8').digest('base64'));
    const hmac = crypto.createHmac('sha256', auth['secret']);
    const signature = hmac.update(util.format('date: %s\ndigest: %s', gmTime, bodyDigest), 'utf8')
      .digest('base64');
    headers = {
      'Authorization': util.format('hmac username=\"%s\", algorithm=\"hmac-sha256\", headers=\"date digest\", '
          + 'signature=\"%s\"', auth['username'], signature),
      'Digest': bodyDigest,
      'Date': gmTime
    }
  }
...
API.prototype._doGetRequestWithAuth = function(url, opts, cb) {
  url += url.indexOf('?') > 0 ? '&' : '?';
  url += 'r=' + _.random(10000, 99999);
  return this._doRequest('get', url, opts, false, cb);
};
